...
public class Renderer2 implements GLSurfaceView.Renderer {
	...
	void render(GL10 gl){
		...
		try {
			...
			renderer2_obj.vertices.position(mPositionOffset);
			GLES20.glVertexAttribPointer(mPositionHandle, mPositionDataSize, GLES20.GL_FLOAT, false, 0, renderer2_obj.vertices);
			GLES20.glEnableVertexAttribArray(mPositionHandle);
			
			
			//FloatBuffer colorBuffer=ByteBuffer.allocate(4*mBytesPerFloat).order(ByteOrder.nativeOrder()).asFloatBuffer();
			//colorBuffer.put(new float[]{1.0f,0.0f,0.0f,1.0f});
			//colorBuffer.position(0);
			//GLES20.glVertexAttribPointer(mColorHandle, mColorDataSize, GLES20.GL_FLOAT, false, 0/*mStrideBytes*/, colorBuffer);
	...
	class ObjFile{
		public FloatBuffer vertices,normals,elements;
